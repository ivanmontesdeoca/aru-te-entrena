...
test("creates alumno with server-generated UUID and normalized email",async()=>{const alumnos=new AlumnosMemory();const service=createAlumnoAdminService({alumnos,usuarios:new UsuariosMemory(),firebase:{createUser:async()=>({uid:"uid"}),deleteUser:async()=>{},revokeRefreshTokens:async()=>{}}});const created=await service.create(base);assert.match(created.Alumno_ID,/^[0-9a-f-]{36}$/);assert.equal(created.Mail,"test@example.com");});

test("creates ALUMNO access and compensates Firebase when Sheets fails",async()=>{const alumno={...base,Mail:"test@example.com",Alumno_ID:"11111111-1111-4111-8111-111111111111"};const deleted:string[]=[];const service=createAlumnoAdminService({alumnos:new AlumnosMemory([alumno]),usuarios:new UsuariosMemory([],true),firebase:{createUser:async()=>({uid:"firebase-test-uid"}),deleteUser:async(uid)=>{deleted.push(uid)},revokeRefreshTokens:async()=>{}}});await assert.rejects(()=>service.createAccess(alumno.Alumno_ID,"NEW@EXAMPLE.COM"),/sheets failed/);assert.deepEqual(deleted,["firebase-test-uid"]);});

test("disable preserves account and revokes Firebase tokens",async()=>{const alumno={...base,Mail:"test@example.com",Alumno_ID:"11111111-1111-4111-8111-111111111111"};const user:Usuario={Usuario_ID:"22222222-2222-4222-8222-222222222222",Alumno_ID:alumno.Alumno_ID,Email:"test@example.com",Rol:"ALUMNO",Activo:true,UID_Auth:"uid"};const usuarios=new UsuariosMemory([user]);const revoked:string[]=[];const service=createAlumnoAdminService({alumnos:new AlumnosMemory([alumno]),usuarios,firebase:{createUser:async()=>({uid:"uid"}),deleteUser:async()=>{},revokeRefreshTokens:async(uid)=>{revoked.push(uid)}}});await service.setAccess(alumno.Alumno_ID,false);assert.equal(usuarios.values[0].Activo,false);assert.deepEqual(revoked,["uid"]);});
